...
var animation = {
    ...
    year: function (className) {
        ...
        for (var i = 0; i <= String(animation.period.year).length - 1; i++) {
            ...
            var Bounce = new gsap.Bounce();
            timeline.to(digit, 0.25, { top: 0, opacity: 1, ease: Bounce.easeOut });
        }
        return year;
    },
    animate: function () {
        ...
        var fullTimeline = new gsap.TimelineMax();
        var triangleStorage = [];
        for (var i = 0; i <= 50 - 1; i++) {
            var timeline = new gsap.TimelineMax({ repeat: -1 });
            ...
            timeline.set(triangle, { scale: animation.range(10, 20) / 10 }, time)
